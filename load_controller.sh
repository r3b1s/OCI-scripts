...
while true; do
    ...
    if (( $(echo "$avg_usage < 20.0" | bc -l) )); then
        echo -e "${GREEN}CPU usage is below 20%, starting additional load generators...${NC}"
        for i in {1..5}; do
            python3 /opt/load-balancer/load_generator.py 0.01 & load_pid=$!
            load_pids+=($load_pid)
        done
    elif (( $(echo "$avg_usage >= 20.0 && $avg_usage < 25.0" | bc -l) )); then
        echo -e "${GREEN}CPU usage is in target range (20-25%), starting one additional load generator...${NC}"
        python3 /opt/load-balancer/load_generator.py 0.01 & load_pid=$!
        load_pids+=($load_pid)
    elif (( $(echo "$avg_usage >= 25.0 && $avg_usage < 35.0" | bc -l) )); then
        if (( ${#load_pids[@]} > 0 )); then
            echo -e "${RED}CPU usage is above 25%, stopping one load generator...${NC}"
            kill ${load_pids[-1]} # kill the last load generator
            unset 'load_pids[${#load_pids[@]}-1]' # remove it from the array
        else
            echo -e "${RED}CPU usage is above 25%, no load generators are running...${NC}"
        fi
    elif (( $(echo "$avg_usage >= 35.0" | bc -l) )); then
        if (( ${#load_pids[@]} > 0 )); then
            echo -e "${RED}CPU usage is above 35%, stopping all load generators...${NC}"
            for pid in "${load_pids[@]}"; do
                kill $pid
            done
            load_pids=()
            ...
            echo -e "${RED}CPU usage is above 35%, no load generators are running...${NC}"
        fi
    fi
    sleep 5
done
